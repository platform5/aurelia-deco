...
import { Store, IpStack, EnsureModel } from '../deco';
...
var SwissdataGlobal = /** @class */ (function () {
    ...
    SwissdataGlobal.prototype.bootstrap = function (config) {
        ...
        this.ensureUsers = this.container.get(EnsureModel);
        this.ensureUsers.init(UserModel, '&autoFetch=profile', { route: '/search-user' });
        this.ensureProfiles = this.container.get(EnsureModel);
        ...
        return this.start().then(function () {
            ...
            var pluckOperator = pluck("language");
            ...
            if (config.enableIpStackAutoDetect && !_this.state.countryCode) {
                ...
                // async operation
                IpStack.autoDetect().then(function (result) {
            ...
            return _this.sdLogin.init(_this.container.get(Store), _this.swissdataApi);
        }).then(function () {
    ...
    SwissdataGlobal.prototype.onAnyLoad = function () {
        ...
        if (this.config.useDynamicModels) {
            return DynamicConfigModel.getAll().then(function (elements) {
                for (var _i = 0, elements_1 = elements; _i < elements_1.length; _i++) {
                    var instance = elements_1[_i];
                    DynamicDataModel.registerModel(instance);
                }
    ...
    SwissdataGlobal.prototype.updateModelsFromApi = function (modelName, ignoreError) {
        ...
        var store = this.container.get(Store);
        store.registerAction('setAppModels', setAppModels);
        return DynamicDataModel.use(modelName).getAll().then(function (el) {
            var elements = el;
